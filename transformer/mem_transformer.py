...
class MemTransformer(nn.Module):
    ...
    def _forward(self, data, style, mems=None):
        ...
        klen = mlen + qlen
        if self.same_length:
            all_ones = data.new_ones(qlen, klen)
            mask_len = klen - self.mem_len
            if mask_len > 0:
                mask_shift_len = qlen - mask_len
            else:
            ...
            dec_attn_mask = torch.triu(
                data.new_ones(qlen, klen), diagonal=1+mlen).byte()[:,:,None]
        ...
        if self.attn_type == 0: # default
            ...
            core_out = self.drop(data + pos_emb[-qlen:])
    ...
    def forward(self, data, style, target, *mems):
        # nn.DataParallel does not allow size(0) tensors to be broadcasted.
        # So, have to initialize size(0) mems inside the model forward.
        # Moreover, have to return new_mems to allow nn.DataParallel to piece
        # them together.
        ...
        tgt_len = target.size(0)
        hidden, new_mems = self._forward(data, style, mems=mems)
